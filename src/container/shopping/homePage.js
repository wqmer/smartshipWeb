import { Divider,Layout, Menu, Breadcrumb,Icon, Button} from 'antd';
    ...
    <Header  style={{ borderBottom:'0.5px solid LightGrey', position: 'fixed', zIndex: 1, width: '100%' ,display:'flex', justifyContent :'space-between',background:'white'}}>
     ...
     <Content  style = { { display:'flex',justifyContent :'space-between'}}> 
           <Menu      
            ...
            >
                  <Menu.Item  key="show"><Icon type ='rise'/>热销</Menu.Item>
                  <Menu.Item  key="dress"><Icon  type ='skin'/>服饰</Menu.Item>
                  <Menu.Item  key="comestic"><Icon type ='gift'/>化妆品</Menu.Item>
           </Menu> 
           <div>
               { window.location.href == 'http://localhost:8080/shop/orderDetail' ? <p >欢迎，游客！</p> :  
               <div>
                   <a style = {{marginRight : '20px'}} onClick = { () => this.props.history.push(`${url}/orderDetail`) } ><Icon style = {{marginRight : '10px'}} type = 'shopping'></Icon>定制订单</a> 
                   <a style = {{marginRight : '20px'}} onClick = { () => this.props.history.push(`${url}/orderTrack`) } ><Icon style = {{marginRight : '10px'}} type = 'search'></Icon>查询订单</a> 
               </div>
              }
           </div>
